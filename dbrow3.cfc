...
<cfcomponent name="dbrow3"
	...
	<cffunction name="init" access="public"
		...
		<!--- Determine which set of validation attribute names to use for
			...
			1. Modern custom attributes, per HTML5 specs. Use these by setting application.dbrow3modernValAttrs = true.
			2. Transitional attributes, implemented after original attributes caused problems, but before we knew
				about data- "namespace" for custom attributes. Use these by setting application.dbrow3legacyValAttrs = false.
			3. Legacy attribute names (default). These cause problems in browsers that support HTML5 form validations.
				Use these by not setting either of the application vars mentioned above.
			- leon 3/09/11 --->
	...
	<!--- I object.  drawPropertyValue() is poorly named.  Because it performs NO output, it should
	instead be called getPropertyValue() - Jared 1/7/09 --->
	<cffunction name="drawPropertyValue" returnType="string" output="no" access="public">
		...
		<cfif structKeyExists(this.stFKMetaData, arguments.propertyname)>
			...
			<cfset foreignColumn = this.stFKMetaData[arguments.propertyname].foreignColumn>
			<cfset foreignTable = this.stFKMetaData[arguments.propertyname].foreignTable>
			...
			<cfif not(len(foreignNameColumn))>
				<cfset objForeign = createObject('component', '#this.objectMap#.#lcase(REReplaceNoCase(foreignColumn, '_?ID$', ''))#').init()>
			...
			<cfif not(len(foreignObjPath))>
				...
				<cfset foreignObjPath = this.objectMap & "." & REReplaceNoCase(foreignColumn, '_?id$', '')>
			</cfif>
			...
			<cfset v.objForeign.load( this[arguments.propertyname] ) >
		...
		<cfelse>
			...
			<cfswitch expression="#stMD.datatype#">
				<cfcase value="bigint,char,date,decimal,float,integer,smallint,time,timestamp,uuid,varchar" delimiters=",">
					...
					<cfelse>
						<cfset formField = HTMLEditFormat(this[arguments.propertyname]) >
					</cfif>
				</cfcase>
				<cfcase value="bit">
					<cfset formField = YesNoFormat( this[arguments.propertyname] ) >
				</cfcase>
				<!--- <cfcase value="binary">
					<cfset formField = '<input type="file" name="#arguments.propertyname#" id="#arguments.propertyname#" #this.getValidationAttribs(arguments.propertyname)# />'>
				</cfcase> --->
				<cfdefaultcase>
	...
	<cffunction name="drawStandardFormField" returnType="string" output="no" access="public">
		...
		<cfif arguments.propertyname eq theID>
			<!--- You shouldn't ever need to manually edit a PK field - leon 2/7/06 --->
			<cfset formField = '<input type="hidden" name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" value="#HTMLEditFormat(this[arguments.propertyname])#" />'>

		<cfelseif structKeyExists(this.stFKMetaData, arguments.propertyname)>
			<!--- This is a foreign key field. Load up a set of the related items and draw a dropdown. - leon 2/7/06 --->
			<cfset foreignColumn = this.stFKMetaData[arguments.propertyname].foreignColumn>
			<cfset foreignTable = this.stFKMetaData[arguments.propertyname].foreignTable>
			...
			<cfif not(len(foreignNameColumn))>
				...
				<cfset objForeign = createObject('component', '#this.objectMap#.#foreignNameGuess#').init()>
				<cfset foreignNameColumn = objForeign.theNameField>
			</cfif>
			<cfif not(len(foreignNameColumn))>
				<cfthrow message="Could not determine name column in foreign key table.">
			...
			<cfset formField = '<select name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" #this.getTabIndexAttr(arguments.propertyname)# #this.getValidationAttribs(arguments.propertyname)#>'>
			<cfif arguments.drawEmptyOption>
				<cfset formField = formField & '<option value=""></option>'>
			</cfif>
			<cfif not(len(this[arguments.propertyname]))>
				<cfset foundMatch = 1>
			</cfif>
			<cfoutput query="rsForeignObjects">
				<cfset formField = formField & '<option value="#rsForeignObjects[foreignColumn][currentRow]#"'>
				<cfif not(foundMatch) and rsForeignObjects[foreignColumn][currentRow] eq this[arguments.propertyname]>
					<cfset foundMatch = 1>
		...
		<cfelse>
			...
			<cfset stMD = this.stColMetaData[arguments.propertyname]>
			<cfswitch expression="#stMD.datatype#">
				<cfcase value="bigint,char,date,decimal,float,integer,smallint,time,timestamp,varchar" delimiters=",">
					<cfif (not(val(stMD.maxLen)) and stMD.datatype eq "varchar") or (stMD.maxLen gte 300)>
						<cfset formField = '<textarea rows="4" cols="50" name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" #this.getTabIndexAttr(arguments.propertyname)# #this.getValidationAttribs(arguments.propertyname)#>#this[arguments.propertyname]#</textarea>'>
					<cfelse>
						<cfif arguments.propertyname contains "password">
						...
						<cfset formField = '<input type="#fieldType#" size="#size#" name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" #this.getTabIndexAttr(arguments.propertyname)# value="#HTMLEditFormat(this[arguments.propertyname])#" #fieldClass# #this.getValidationAttribs(arguments.propertyname)# />'>
					</cfif>
				</cfcase>
				<cfcase value="other">
					<cfset formField = '<input type="hidden" name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" value="#HTMLEditFormat(this[arguments.propertyname])#" />'>
				...
				<cfcase value="bit">
					<cfset formField = '
							<input type="checkbox" name="#arguments.identifierPrefix##arguments.propertyname#_checkbox" id="#arguments.identifierPrefix##arguments.propertyname#_checkbox" value="1" #this.getTabIndexAttr(arguments.propertyname)# #this.getValidationAttribs(arguments.propertyname)# #iif(len(this[arguments.propertyname]) and this[arguments.propertyname], de("checked"), de(""))# onclick="document.getElementById(''#arguments.propertyname#'').value=this.checked;" />
							<input type="hidden" name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" value="#iif(len(this[arguments.propertyname]) and this[arguments.propertyname], 1, 0)#" />
						'>
				</cfcase>
				<cfcase value="binary">
					<cfset formField = '<input type="file" name="#arguments.identifierPrefix##arguments.propertyname#" id="#arguments.identifierPrefix##arguments.propertyname#" #this.getTabIndexAttr(arguments.propertyname)# #this.getValidationAttribs(arguments.propertyname)# />'>
				</cfcase>
				<cfdefaultcase>
		...
		<cfif arguments.appendErrorSpan>
			<cfset formField = formField & drawErrorField( propertyname = arguments.propertyname, errorText = arguments.errorText, identifierPrefix = arguments.identifierPrefix ) >
		</cfif>
	...
	<cffunction name="edit" returnType="void" output="yes" access="remote">
		...
		<html>
			<body>
				...
				#this.drawForm(bIncludeValScript = 1, jsIncludePath = '#application.appRootURL##iif(right(application.appRootURL,1) eq '/', de(''), de('/'))#js/', goto = arguments.goto, arErrors = arErrors)#
				<cfif 0>
					DEBUG<br />
					<cfdump var="#this.stMany#">
					<cfloop collection="#this.stMany#" item="v.i">
